...
public class CustomCmdCheckColor(TORWLPlugin plugin, uint id) : PlayerCustomRpc<TORWLPlugin, CustomColorData>(plugin, id)
{
    ...
    private static bool VerifyColor(byte requestedColor, byte requestedGradient, byte playerId)
    {
        ...
        {
            Logger<TORWLPlugin>.Error($"Error getting gradient for player {data.PlayerName}");
