...
namespace TORWL.Features.Managers;
...
public static class GradientManager
{
    ...
    static GradientManager()
    {
        try
        {
            GradientConfig = PluginSingleton<TORWLPlugin>.Instance.Config.Bind("Gradient", "Secondary", 0, "Gradient ID");
        }
        catch (Exception e)
        {
            Logger<TORWLPlugin>.Warning(e.ToString());
        }
    }
